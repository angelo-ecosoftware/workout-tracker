...
import { UserProfile, Workout, Session, WorkoutSet, Exercise } from '../types/index.ts';
import { SessionEngine, SetLogger } from '../engine/index.ts';
...
export async function logSessionCompletion(
  ...
  if (sessionErr || !insertedSession) {
    throw new Error(`Failed to log session: ${sessionErr?.message}`);
  }
