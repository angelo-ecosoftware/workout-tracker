...
export async function importAllLogs(userId: string, data: any) {
  ...
  try {
    if (data.workouts && Array.isArray(data.workouts) && data.workouts.length > 0) {
      const sanitizedWorkouts = data.workouts.map((w: any) => ({
        ...w,
        user_id: userId,
      }));
    ...
    if (data.sets && Array.isArray(data.sets) && data.sets.length > 0) {
      const cleanSets = data.sets.map((s: any) => {
    ...
    if (data.bodyMeasurements && Array.isArray(data.bodyMeasurements) && data.bodyMeasurements.length > 0) {
      const sanitizedBody = data.bodyMeasurements.map((b: any) => ({
        ...b,
        user_id: userId,
      }));
    ...
    if (data.user && typeof data.user === 'object') {
      await supabase
        .from('users')
        .update({
          last_completed_workout_order: data.user.last_completed_workout_order ?? 0,
          last_set_summary_per_exercise: data.user.last_set_summary_per_exercise ?? {},
          metrics: data.user.metrics ?? undefined,
        })
        .eq('user_id', userId);
    }
