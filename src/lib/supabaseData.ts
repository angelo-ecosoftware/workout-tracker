...
export async function getUserProgressState(userId: string) {
  ...
  return {
    profile: {
      ...
      trainingDays: data.training_days || data.metrics?.trainingDays || [],
      sessionDurationMinutes: data.session_duration_minutes || data.metrics?.sessionDurationMinutes || undefined,
      goals: data.goals || data.metrics?.goals || [],
      injuriesNotes: data.injuries_notes || data.metrics?.injuriesNotes || data.metrics?.bodyMeasurementsNotes || undefined,
      ...
      metrics: {
        ...
        dateOfBirth: data.date_of_birth || data.metrics?.dateOfBirth || localMetrics?.dateOfBirth,
        gender: data.gender || data.metrics?.gender || localMetrics?.gender,
        fitnessLevel: data.fitness_level || data.metrics?.fitnessLevel || localMetrics?.fitnessLevel,
        trainingLocation: data.training_location || data.metrics?.trainingLocation || localMetrics?.trainingLocation,
        ...
        goals: data.goals || data.metrics?.goals || localMetrics?.goals,
        injuriesNotes: data.injuries_notes || data.metrics?.injuriesNotes || data.metrics?.bodyMeasurementsNotes || localMetrics?.injuriesNotes,
      },
    } as UserProfile,
    isNewUser: false,
  };
...
export { initializeUser, saveOnboardingProfile } from './db/users.ts';
