...
CREATE POLICY "Users can view sessions"
  ...
  FOR SELECT
  TO anon, authenticated
  USING (auth.uid()::text = user_id OR status = 'completed');
...
CREATE POLICY "Allow insert food_items"
  ...
  WITH CHECK (
    ...
    (is_custom = true AND auth.uid()::text = user_id)
    OR
    -- If public store food: user_id is null/community
    (is_custom = false AND (user_id IS NULL OR user_id = 'community' OR auth.uid()::text = user_id))
  );
